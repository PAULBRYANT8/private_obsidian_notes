...
| 维度           | v0.2.1                                                        | v0.2.2                                                  |
| ------------ | ------------------------------------------------------------- | ------------------------------------------------------- |
| 切分时机         | forward 时通过 context manager 隐式切分                              | 数据加载后显示切分                                               |
| 序列分配策略       | 连续均匀分块（无负载均衡）                                                 | HeadTail 交错配对（SDPAA）/ PTRR 动态均衡（Flex）                   |
...
| 模型侧注入        | 隐式（context manager拦截）                                         | 显示 parallizer_module + DTensor 并行化计划                    |
