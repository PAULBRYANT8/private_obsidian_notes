...
KV 传递方式：环形传递（Ring）。PyTorch 实现中提供两种通信原语：默认的 All-Gather 模式（一次性收集所有 KV 再按轮取片）和 AllToAll 模式（逐轮传递，显存更低），详见第二章第(3)节。
...
```python
  # llama3 model.py 
  self.freqs_cis = precompute_freqs_cis(...)  # shape: [max_seq_len, head_dim//2]
```
...
# 三、RoPE（Rotary Position Embedding，旋转位置编码）详解
...
- 绝对位置编码（BERT/GPT-1/2 做法）
  ...
  问题：
    - 最大序列长度固定（`position_embedding` 是查找表，例如 BERT 训练时最多 512 个位置，GPT-2 最多 1024 个位置）
...
**通俗来说就是：** “我们希望找到一种方法，把词汇的**绝对位置**编码进去；但当模型计算它们之间的注意力分数时，数学运算的结果却能自然而然地只与它们的**相对距离**有关。”

### （1） RoPE 是如何解决的（“旋转”的奥秘）
...
**请注意：当你改变绝对位置时，向量本身会发生旋转；但是，只要两个位置之间的<mark style="background:#d3f8b6">距离</mark>（即 $m-n$ 的差值）保持不变，这两个向量之间的夹角（即它们算出来的注意力分数）就会完全保持锁定。**
...
### Q：HeadTail 会打乱 token 顺序，计算结果还有意义吗？

tokens 确实被重排了，但计算结果仍然正确。核心在于：token 被打乱顺序的同时，positions 张量也同步打乱——模型所有依赖"位置"的操作，用的都是 positions 张量而不是本地下标。

**第一步：cp_shard 同时重排 inputs、labels、positions**

```python
# prepare_context_parallel_input（context_parallel.py:122-131）
positions = torch.arange(0, inputs.shape[1]).expand(inputs.shape)
             # positions = [[0, 1, 2, 3, 4, 5, 6, 7]]（全局绝对位置）

(inputs, labels, positions), _ = cp_shard(
    cp_mesh, (inputs, labels, positions), ...   # 三者用同一个 load_balancer 同步重排
)
```

HeadTail 重排后（seq_len=8，CP=4）：

| rank | 持有的 token       | 持有的 positions |
| ---- | ---------------- | -------------- |
| 0    | token[0], token[7] | [0, 7]         |
| 1    | token[1], token[6] | [1, 6]         |
| 2    | token[2], token[5] | [2, 5]         |
| 3    | token[3], token[4] | [3, 4]         |

token 和 positions 始终一一对应，rank 永远不会"搞混"哪个 token 在哪个位置。

**第二步：RoPE 用绝对 positions 索引，而非本地下标**

```python
# reshape_for_broadcast（model.py:124）
freqs_cis = freqs_cis[positions.squeeze(0)]
# rank 0 取的是 freqs_cis[[0, 7]] ← 位置 0 和位置 7 的旋转因子
# 不是 freqs_cis[0:2]            ← 那是位置 0 和位置 1，完全错误
```

即使 rank 0 本地只有 2 个 token，它们也分别拿到了全局位置 0 和全局位置 7 的旋转因子，位置编码完全正确。

**第三步：Causal Mask 基于绝对位置判断**

Ring Attention 旋转 KV 的过程中，每个 rank 知道自己 Q 的绝对 positions 和当前轮次 KV 的绝对 positions，causal mask 规则为 `Q 的绝对位置 ≥ K 的绝对位置`。

以 rank 0（持有 positions=[0, 7]）处理来自 rank 3 的 KV（positions=[3, 4]）为例：

| 操作                           | 结果            |
| ---------------------------- | ------------- |
| token@pos0 attend token@pos3 | ✗ 被 mask（0 < 3） |
| token@pos0 attend token@pos4 | ✗ 被 mask（0 < 4） |
| token@pos7 attend token@pos3 | ✓ 允许（7 ≥ 3）   |
| token@pos7 attend token@pos4 | ✓ 允许（7 ≥ 4）   |

判断依据始终是绝对位置，与这两个 token 在 rank 0 本地的顺序（第 0 个 / 第 1 个）毫无关系。

**第四步：Loss 计算正确，因为 labels 同步重排**

```python
(inputs, labels, positions), _ = cp_shard(...)
# inputs 和 labels 用同一个 load_balancer → 重排方式完全一致
# rank 0: inputs=[token0, token7], labels=[label0, label7]
# 对应关系不乱 → cross_entropy(output[i], labels[i]) 仍然正确
```

**小结**：打乱 token 本地顺序本身不破坏正确性，因为模型所有"位置敏感"的操作——RoPE 编码、Causal Mask、Loss 配对——用的都是随 token 一起打乱的 positions 张量，而不是本地下标。可以把 HeadTail 理解成一个"带标签的散货箱"：货物（token）的物理摆放顺序变了，但每件货物上都贴着正确的位置标签（positions），所有流程按标签操作，结果与按顺序摆放完全等价。

<mark style="background:#b1ffff">SDPA 是一个无状态的逐对函数，输入 (Q_i, pos_i, K_j, pos_j) → 输出贡献，与其他 pair 无关，与物理排列无关</mark>。
...
| 模型侧注入        | 隐式（context manager拦截）                                         | 显式 parallelize_module + DTensor 并行化计划                   |
| API 层次       | torch.distributed.tensor.experimental.context_parallel（旧 API） | _ContextParallel + _context_parallel_shard（新 API）       |
| 负载均衡配置       | 无                                                             | context_parallel_load_balancer = "headtail"/"ptrr"/None |
| PP 兼容性       | 需要特殊处理 freqs_cis buffer（每个 PP stage 单独处理）                     | 无需特殊处理，通过 positions 统一解决                                |

## 4.5、修改带来的好处
### （1）解决 Causal Attention 的负载不均衡

这是最核心的改进。原有均匀切分下（以总序列长度 S、CP degree N 为例）：
- rank 0 持有 token 0 ~ S/N-1，每个 token 只能 attend 到其 chunk 内的前序 token，总计算量 ∝ (S/N)² / 2
- rank N-1 持有 token (N-1)S/N ~ S-1，每个 token 可 attend 到全部前序 token，总计算量 ∝ S/N × S
...
v0.2.1 中必须将模型内部的 RoPE 缓存 freqs_cis 作为 buffer 传入切分，在 PP（Pipeline Parallel）下每个 stage 都要单独处理，逻辑复杂且易出错。v0.2.2 改为切分 positions 索引，模型内部通过`freqs_cis[positions]` 的方式索引取值，完全解耦了 RoPE 与 CP 的交互。
...
## 4.6、参考文献
  ...
  以及 Megatron-LM 中序列并行的相关参考（该论文核心贡献为 activation recomputation 减少与序列并行，Megatron 的 CP 实现也采用了类似的交错分配策略）：
